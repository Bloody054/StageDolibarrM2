...
$questionnaire->fetch($id);

llxHeader('', $langs->trans('Questionnaire'));


$head = questionnaire_prepare_head($object);
dol_fiche_head($head, 'exporter', $langs->trans("Questionnaire"), 0, 'questionnaire@questionnaire');

$url = dol_buildpath('/questionnaire/list.php', 1).'?restore_lastsearch_values=1';
$linkback = '<a href="' . $url . '">' . $langs->trans("BackToList") . '</a>';

$morehtml = ' - '.$questionnaire->title;
dol_banner_tab($questionnaire, 'ref', $linkback, 1, 'ref', 'ref',$morehtml);

print '<div class="fichecenter">';
print '<div class="underbanner clearboth"></div>';

print '<br>';
$export = $questionnaire->export_sql_table($questionnaire->id,1,0);
print '<code>'.$export.'</code>';
