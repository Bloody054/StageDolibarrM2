...
/**
 *  \file		htdocs/questionnaire/tpl/linkedobjectblock.tpl.php
...
?>
    <tr class="<?php echo $trclass; ?>"  data-element="<?php echo $objectlink->element; ?>"  data-id="<?php echo $objectlink->id; ?>" >
        <td class="linkedcol-element" ><?php echo $langs->trans("Questionnaire"); ?>
        <?php if(!empty($showImportButton) && $conf->global->MAIN_ENABLE_IMPORT_LINKED_OBJECT_LINES)
            {
                $url = dol_buildpath('/questionnaire/card.php', 1).'?id='.$objectlink->id;
                //$url = DOL_URL_ROOT.'/comm/questionnaire/card.php?id='.$objectlink->id;
    	...
    	<td class="linkedcol-action" align="right"><a href="<?php echo $_SERVER["PHP_SELF"].'?id='.$object->id.'&action=dellink&dellinkid='.$key.'&token='.newToken(); ?>"><?php echo img_picto($langs->transnoentitiesnoconv("RemoveLink"), 'unlink'); ?></a></td>
    </tr>
<?php
}
?>
