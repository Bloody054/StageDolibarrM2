...
dol_include_once("/product/entrepot/class/entrepot.class.php");
...
$search_fichier = isset($_POST['search_fichier']) || isset($_GET['search_fichier']) ? GETPOST('search_fichier','int') : -1;
...
if (!empty($conf->questionnaire->enabled))
{
    ...
    $fields = $questionnaire->getAllFields();
}
...
if (count($fields)) {
    foreach ($fields as $code => $field) {
        if ($field->type == 'date' || $field->type == 'datetime') {
            $search_form[$field->code.'day'] = GETPOST('search_'.$field->code.'day',"int");
            $search_form[$field->code.'month'] = GETPOST('search_'.$field->code.'month',"int");
            $search_form[$field->code.'year'] = GETPOST('search_'.$field->code.'year',"int");
        } else if ($field->type == 'checkbox') {
            $s = GETPOST('search_'.$field->code);
            $search_form[$field->code] = is_array($s) ? implode(',', $s) : $s;
        } else if (in_array($field->type, array('list', 'table', 'radio'))) {
            $search_form[$field->code] = isset($_POST['search_'.$field->code]) || isset($_GET['search_'.$field->code]) ? GETPOST('search_'.$field->code, 'int') : -1;
        } else {
            $search_form[$field->code] = GETPOST('search_'.$field->code);
        }   
    }
}
...
$productgeneric = new Product($db);
$warehousegeneric = new Entrepot($db);
...
$arrayfields['with_file'] = array('label'=>$langs->trans("AvecFichier"), 'checked'=>1);
...
if (count($fields))
{
    foreach ($fields as $code => $field)
    {
        if (($field->type == 'file' && $user->rights->reponse->telecharger) || $field->type != 'file') {
            $arrayfields[$code] = array('label'=>$field->label, 'checked'=>0);
        }
    }
}
...
if (empty($reshook))
{
    ...
    $permtomerge = $user->rights->reponse->fusionner;
    $permtomassmerge = $user->rights->reponse->fusionner_masse;
...
$sql.= ' s.rowid, s.ref, s.datec, s.fk_questionnaire, s.tms, s.user_author_id, s.envoi_ar, s.entity';
$sql.= ", u.rowid as user_id, u.lastname as user_lastname, u.firstname as user_firstname";
if (!empty($conf->questionnaire->enabled))
{
    $sql.= ", f.rowid as form_id, f.ref as form_ref, f.title as form_title ";
}
...
if (!empty($conf->questionnaire->enabled))
{
    ...
    $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."questionnaireval".$search_fk_questionnaire." as fv ON fv.fk_reponse = s.rowid";
}
...
if (count($fields)) {
    foreach ($fields as $code => $field) {
        if (in_array($field->type, array('string', 'text', 'int'))) {
            $s = isset($search_form[$field->code]) ? $search_form[$field->code] : '';
            if ($s) $sql .= natural_search($code, $s);
        } else if (in_array($field->type, array('checkbox'))) {
            $s = isset($search_form[$field->code]) ? $search_form[$field->code] : '';
            if ($s) $sql.= " AND ".$code." IN (".$s.")";
        } else if (in_array($field->type, array('list', 'table', 'radio'))) {
            $s = isset($search_form[$field->code]) ? $search_form[$field->code] : -1;
            if ($s >= 0) $sql.= " AND ".$code." = ".intval($s);
        } else if (in_array($field->type, array('date', 'datetime'))) {
            $searchd = isset($search_form[$field->code.'day']) ? $search_form[$field->code.'day'] : 0;
            $searchm = isset($search_form[$field->code.'month']) ? $search_form[$field->code.'month'] : 0;
            $searchy = isset($search_form[$field->code.'year']) ? $search_form[$field->code.'year'] : 0;

            if ($searchm > 0)
            {
                if ($searchy > 0 && empty($searchd))
                $sql.= " AND ".$code." BETWEEN '".$db->idate(dol_get_first_day($searchy, $searchm, false))."' AND '".$db->idate(dol_get_last_day($searchy, $searchm, false))."'";
                else if ($searchy > 0 && ! empty($searchd))
                $sql.= " AND ".$code." BETWEEN '".$db->idate(dol_mktime(0, 0, 0, $searchm, $searchd, $searchy))."' AND '".$db->idate(dol_mktime(23, 59, 59, $searchm, $searchd, $searchy))."'";
                else
                $sql.= " AND date_format(".$code.", '%m') = '".$searchm."'";
            }
            else if ($searchy > 0)
            {
                $sql.= " AND ".$code." BETWEEN '".$db->idate(dol_get_first_day($searchy, 1, false))."' AND '".$db->idate(dol_get_last_day($searchy, 12, false))."'";
            }
        } else if ($field->type == 'file') {
            if ($search_fichier == 1) {
                $sql.= " AND NOT (".$code." IS NULL OR ".$code." LIKE '')";
            } else if ($search_fichier == 0) {
                $sql.= " AND (".$code." IS NULL OR ".$code." LIKE '')";
            }
        }
    }
}
...
if(!empty($search_fk_questionnaire)){
    $resql = $db->query($sql);
...
if ($search_fichier >= 0)               $param.='&search_fichier='.urlencode($search_fichier);

if (count($fields)) {
    foreach ($fields as $code => $field) {
        if ($field->type == 'date' || $field->type == 'datetime') {
            $s = isset($search_form[$field->code.'day']) ? $search_form[$field->code.'day'] : '';
            if ($s) $param.='&search_'.$field->code.'day='.urlencode($s);
            $s = isset($search_form[$field->code.'month']) ? $search_form[$field->code.'month'] : '';
            if ($s) $param.='&search_'.$field->code.'month='.urlencode($s);
            $s = isset($search_form[$field->code.'year']) ? $search_form[$field->code.'year'] : '';
            if ($s) $param.='&search_'.$field->code.'year='.urlencode($s);
        }  else if (in_array($field->type, array('list', 'table', 'checkbox'))) {
            $s = isset($search_form[$field->code]) ? $search_form[$field->code] : -1;
            if ($s >= 0) $param.='&search_'.$field->code.'='.urlencode($s);
        } else {
            $s = isset($search_form[$field->code]) ? $search_form[$field->code] : '';
            if ($s) $param.='&search_'.$field->code.'='.urlencode($s);
        }
    }
}
...
if(empty($search_fk_questionnaire)){
    echo $langs->trans('YouNeedSelectAForm');
    $questionnaire = new Questionnaire($db);
...
// Avec fichier
if (! empty($arrayfields['with_file']['checked']))
{
    print '<td class="liste_titre">';
    print $form->selectyesno('search_fichier', $search_fichier, 1, 0, 1);
    print '</td>';
}
...
if (count($fields) > 0) {
    foreach ($fields as $code => $line) {
        if (!empty($arrayfields[$code]['checked'])) {

            print '<td class="liste_titre">';
            if ($line->crypted) {
                print '&nbsp;';
            } else {
                if (in_array($line->type, array('date', 'datetime'))) {
                    $s = isset($search_form[$line->code.'day']) ? $search_form[$line->code.'day'] : '';
                    if (! empty($conf->global->MAIN_LIST_FILTER_ON_DAY)) print '<input class="flat width25 valignmiddle" type="text" maxlength="2" name="search_'.$line->code.'day" value="'.$s.'">';

                    $s = isset($search_form[$line->code.'month']) ? $search_form[$line->code.'month'] : '';
                    print '<input class="flat width25 valignmiddle" type="text" maxlength="2" name="search_'.$line->code.'month" value="'.$s.'">';

                    $s = isset($search_form[$line->code.'year']) ? $search_form[$line->code.'year'] : -1;
                    $formother->select_year($s, 'search_'.$line->code.'year',1 , 20, 5);
                } else if (in_array($line->type, array('int', 'text', 'string'))) {
                    $s = isset($search_form[$line->code]) ? $search_form[$line->code] : '';
                    print '<input class="flat" size="6" type="text" name="search_'.$line->code.'" value="'.$s.'">';

                } else if (in_array($line->type, array('product'))) {
                    $s = isset($search_form[$line->code]) ? $search_form[$line->code] : '';
                    print 'Filtre product <input class="flat" size="6" type="text" name="search_'.$line->code.'" value="'.$s.'">';
                
                } else if (in_array($line->type, array('radio', 'list', 'table', 'checkbox'))) {
                    $options = array();
                    $params = explode("\r\n", $line->param);
                    if ($line->type == 'table') {
                        $table = $params[0];
                        ...
                        $sqlform = "SELECT $rowid, $label FROM ".MAIN_DB_PREFIX."$table WHERE active = 1";
                        $resform = $db->query($sqlform);
                        if ($resform)
                        {
                            $numform = $db->num_rows($resform);

                            $iform = 0;
                            while ($iform < $numform)
                            {
                                $objform = $db->fetch_object($resform);
                                $options[$objform->{$rowid}] = $objform->{$label};
                                $iform++;
                            }
                        }
                    } else {
                    ...
                    $s = isset($search_form[$line->code]) ? $search_form[$line->code] : -1;
                    if ($line->type == 'checkbox' && $s) {
                        $s = explode(',', $s);
                    }
                    print $line->type == 'checkbox' ? $form->multiselectarray('search_'.$line->code, $options, $s) : $form->selectarray('search_'.$line->code, $options, $s, 1);
                } else {
                    print '&nbsp;';
                }
            }
...
if (! empty($arrayfields['with_file']['checked']))            print_liste_field_titre($arrayfields['with_file']['label'],$_SERVER["PHP_SELF"],'','',$param,'',$sortfield,$sortorder);
...
if (count($fields) > 0) {
    foreach ($fields as $code => $field) {
        if (!empty($arrayfields[$code]['checked'])) {
            print_liste_field_titre($arrayfields[$code]['label'],$_SERVER["PHP_SELF"],$code,'',$param,'',$sortfield,$sortorder);
        }
    }
}
    ...
    if (! empty($arrayfields['with_file']['checked']))
    {
        ...
        if (count($fields) > 0) {
            foreach ($fields as $code => $line) {
                ...
                if ($line->type == 'file') {
                    $value = isset($obj->{$line->code}) ? $obj->{$line->code} : null;
                    $with_file = $with_file || !empty($value);
                }
            }
        }
    ...
    if (count($fields) > 0) {
        foreach ($fields as $code => $line) {
            if (!empty($arrayfields[$code]['checked'])) {
                ...
                $line->fk_reponse = $obj->rowid;
                $line->value = $value;

                if (!empty($line->crypted)) {
                    if (strpos($line->value, ':') !== false && ($user->rights->reponse->decrypter || $obj->user_author_id == $user->id)) {
                        list($key, $value) = explode(':', $line->value);
                        $line->value  = mb_dol_decode($value, $key);
                    }
                }

                $value = '';
                if (($line->type == 'file' && $user->rights->reponse->telecharger) || $line->type != 'file') {
                    $value = $questionnaireform->valueField($line);
                }

                if (($line->type == 'product')){
                    $productgeneric->fetch($value);
                    $value = $productgeneric->getNomUrl();                                                           
                }

                if (($line->type == 'warehouse')){
                    if($value>0){
                      $warehousegeneric->fetch($value);
                      $value = $warehousegeneric->getNomUrl();  
                    }else{
                        $value='';
                    }
