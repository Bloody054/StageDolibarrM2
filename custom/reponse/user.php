...
$contextpage = GETPOST('contextpage','aZ') ? GETPOST('contextpage','aZ') : 'userreponselist';
...
$arrayfields=array(
    ...
    's.for_human'=>array('label'=>$langs->trans("ForHuman"), 'checked'=>1),
...
if (empty($reshook))
{
    ...
    if ($action == 'confirm_linkreponse' && !GETPOST('cancel','alpha'))
    {
        $fk_reponse = GETPOST('fk_reponse', 'int');
        if ($object->fetch($fk_reponse) > 0) {
            $object->user_author_id = $fuser->id;
            if ($object->update($user) > 0) {
                setEventMessages($langs->trans('ReponseLinkedToUser'), array(), 'mesgs');
            } else {
                setEventMessages($object->error, array(), 'errors');
            }
        } else {
            setEventMessages($langs->trans('NoReponse'), array(), 'errors');
        }
    }
}
...
$sql.= ' WHERE s.is_draft = 0 AND s.entity IN ('.getEntity('reponse').') AND s.user_author_id = '.$id;
...
if ($resql)
{
	...
	dol_fiche_head($head, 'reponse', $title, -1, 'user');

	dol_banner_tab($fuser,'id', '', $user->rights->user->user->lire);
	...
	if ($search_fk_questionnaire)      		$param.='&search_fk_questionnaire='.urlencode($search_fk_questionnaire);
	...
	$arrayofmassactions =  array(
		...
		'download' => $langs->trans("MassDownload")
	);
	...
	{
		print $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$id, $langs->trans("ConfirmMassConfirmation"), $langs->trans("ConfirmMassConfirmationQuestion", count($toselect)), "confirm", null, '', 0, 200, 500, 1);
	}

    // Confirmation to link
    if ($action == 'linkreponse' && !empty($conf->reponse->enabled)) {

        $formquestion = array();

        $reponse = new Reponse($db);
        $reponses = $reponse->liste_array();

        $out = $form->selectarray('fk_reponse', $reponses, GETPOST('fk_reponse'), 0, 0, 0, 'style="width: 450px"', 0, 0, 0, '', '', 1);

        $formquestion = array(
            array('type' => 'hidden','name' => 'fk_reponse','label' => $langs->trans("Reponse"),'value' => $out)
        );

        print $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $fuser->id, $langs->trans('LierReponse'), $langs->trans('ConfirmLierReponse'), 'check_linkreponse', $formquestion, 0, 0, 250, 650);
    }

    if ($action == 'check_linkreponse' && !empty($conf->reponse->enabled)) {

        $fk_reponse = GETPOST('fk_reponse', 'int');
        if ($object->fetch($fk_reponse) > 0) {

            $formquestion = array(
                array('type' => 'hidden','name' => 'fk_reponse','label' => $langs->trans("Reponse"),'value' => GETPOST('fk_reponse', 'int'))
            );

            $u = new User($db);
            $userurl = '';
            $useremail = '';
            if ($object->user_author_id > 0 && $u->fetch($object->user_author_id)) {
                $userurl = $u->getNomUrl();
                $useremail = $u->email;
            }
            $confirm = $langs->trans('ConfirmLierReponseDetails', $object->getNomUrl(), dol_print_date($object->datec, 'day'), $userurl, $useremail);
            $confirm.= '<br />';
            $confirm .= $object->user_author_id ==  $conf->global->CONNECT_AGENT_ID ? $langs->trans('ConfirmLierReponseFromAgent') : $langs->trans('ConfirmLierReponseFromUser');

            print $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $fuser->id, $langs->trans('ConfirmLierReponse'), $confirm, 'confirm_linkreponse', $formquestion, 0, 0, 250, 650);
    ...
    */
    ...
    if (! empty($conf->reponse->enabled)) {
        print '<div class="inline-block divButAction"><a class="butAction" href="' . $_SERVER["PHP_SELF"] . '?id=' . $fuser->id . '&action=linkreponse">' . $langs->trans('LierReponse') . '</a></div>';
    }
